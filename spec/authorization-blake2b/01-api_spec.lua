...
for _, strategy in helpers.each_strategy() do
  describe("Plugin: authorization-blake2b (API) [#" .. strategy .. "]", function()
    local consumer
    local admin_client
    local bp
    local db
    ...
    describe("/consumers/:consumer/authorization-blake2b", function()
      describe("POST", function()
        after_each(function()
          db:truncate("keyauth_credentials")
        end)
        it("creates a authorization-blake2b credential with key", function()
          local res = assert(admin_client:send {
            method  = "POST",
            path    = "/consumers/bob/authorization-blake2b",
            body    = {
              key   = "1234"
            },
            headers = {
        ...
        it("creates a authorization-blake2b auto-generating a unique key", function()
          local res = assert(admin_client:send {
            method  = "POST",
            path    = "/consumers/bob/authorization-blake2b",
            body    = {},
        ...
        it("creates a authorization-blake2b credential with tags", function()
          local res = assert(admin_client:send {
            method  = "POST",
            path    = "/consumers/bob/authorization-blake2b",
            body    = {
        ...
        it("creates a authorization-blake2b credential with a ttl", function()
          local res = assert(admin_client:send {
            method  = "POST",
            path    = "/consumers/bob/authorization-blake2b",
            body    = {
          ...
          local res = assert(admin_client:send {
            method  = "GET",
            path    = "/consumers/bob/authorization-blake2b/" .. id,
          })
    ...
    describe("/consumers/:consumer/authorization-blake2b/:id", function()
      local credential
      ...
      describe("GET", function()
        it("retrieves authorization-blake2b credential by id", function()
          local res = assert(admin_client:send {
            method  = "GET",
            path    = "/consumers/bob/authorization-blake2b/" .. credential.id
          })
          local body = assert.res_status(200, res)
          local json = cjson.decode(body)
          assert.equal(credential.id, json.id)
        end)
        it("retrieves authorization-blake2b credential by key", function()
          local res = assert(admin_client:send {
            method  = "GET",
            path    = "/consumers/bob/authorization-blake2b/" .. credential.key
          })
          local body = assert.res_status(200, res)
          local json = cjson.decode(body)
          assert.equal(credential.id, json.id)
        ...
        it("retrieves credential by id only if the credential belongs to the specified consumer", function()
          ...
          res = assert(admin_client:send {
            method = "GET",
            path   = "/consumers/alice/authorization-blake2b/" .. credential.id
          })
          assert.res_status(404, res)
        end)
        it("authorization-blake2b credential contains #ttl", function()
      ...
      describe("PUT", function()
        ...
        it("creates a authorization-blake2b credential with key", function()
          local res = assert(admin_client:send {
            method  = "PUT",
            path    = "/consumers/bob/authorization-blake2b/1234",
            body    = {},
            headers = {
              ["Content-Type"] = "application/json"
            }
        ...
        it("creates a authorization-blake2b credential auto-generating the key", function()
          local res = assert(admin_client:send {
            method  = "PUT",
            path    = "/consumers/bob/authorization-blake2b/c16bbff7-5d0d-4a28-8127-1ee581898f11",
            body    = {},
            headers = {
              ["Content-Type"] = "application/json"
            }
      ...
      describe("DELETE", function()
        ...
        describe("errors", function()
          it("returns 400 on invalid input", function()
            local res = assert(admin_client:send {
              method  = "DELETE",
              path    = "/consumers/bob/authorization-blake2b/blah"
            })
            assert.res_status(404, res)
          end)
          it("returns 404 if not found", function()
            local res = assert(admin_client:send {
              method  = "DELETE",
              path    = "/consumers/bob/authorization-blake2b/00000000-0000-0000-0000-000000000000"
            })
            assert.res_status(404, res)
          end)
        end)
    ...
    describe("/key-auths", function()
      ...
      describe("POST", function()
        ...
        it("does not create authorization-blake2b credential when missing consumer", function()
          local res = assert(admin_client:send {
            method = "POST",
            path = "/key-auths",
            body = {
        ...
        it("creates authorization-blake2b credential", function()
          local res = assert(admin_client:send {
            method = "POST",
            path = "/key-auths",
            body = {
